...
class Criptografia:
    ...
    def KDF_password_storage(self, name, contra, tokens, offers, public_key):
        ...
        totp_key = public_key.encrypt(
                self.TOKEN_code_authenticator_qr(name).encode('ascii'),
                padding.OAEP(
        ...
        user_dict = {"user_name": base64.b64encode(name).decode('ascii'),
                     "user_pass": base64.b64encode(derived_key).decode('ascii'),
                     "user_tokens": base64.b64encode(tokens).decode('ascii'),
                     "user_total_tokens_offered": base64.b64encode(offers).decode('ascii'),
    ...
    def KDF_verify_user_name(self, name: str) -> bool:
        ...
        for dicti in self.user_data_list:
            if(name == self.decrypt_with_private_key(dicti["user_name"])):
                return True
            self.n_dict += 1
        return False
    ...
    def KDF_verify_password(self, contra: str) -> bool:
        ...
        try:
            KDF.verify(contra, base64.b64decode(self.user_data_list[self.n_dict]["user_pass"]))
    ...
    def TOKEN_verify_code(self, input_key: str) -> bool:
        ...
        totp_del_servidor = pyotp.totp.TOTP(self.decrypt_with_private_key(self.user_data_list[self.n_dict]["user_totp"]))
